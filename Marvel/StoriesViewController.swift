...
class StoriesViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        /*Alamofire.request(String(format: "https://gateway.marvel.com/v1/public/characters/%i/stories?ts=%i&apikey=%@&hash=%@&limit=%i", id, timestamp, public_key, md5_hash, request_limit), method: .get).validate().responseJSON { response in
            switch response.result {
            case .success(let value):
                let json = JSON(value)
                print("JSON: \(json)")
                
                print(json["data"]["results"].count)
                
                for i in 0..<json["data"]["results"].count
                {
                    print("!!!", json["data"]["results"][i])
                    
                    self.idsList.append(json["data"]["results"][i]["id"].int!);
                    self.storiesList.append(json["data"]["results"][i]["title"].string!);
                ...
                for (index, value) in self.storiesList.enumerated() {
                    print("Item \(index + 1): \(value)")
                }
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.label?.text = self.storiesList[indexPath.row];
